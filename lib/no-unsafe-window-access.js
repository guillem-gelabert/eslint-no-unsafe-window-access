...
const isClientGuard = (node) => {
    if (isImportMetaClient(node) ||
...
const isInClientGuard = (node, ancestors) => {
...
const rule = createRule({
    ...
    meta: {
        type: "problem",
        docs: {
            description: "Disallow unsafe access to the `window` object without client-side guards",
        },
        fixable: "code",
        schema: [],
        messages: {
            unsafeWindowAccess: "Unsafe access to `window` object. Wrap in a client-side check like `if (typeof window !== 'undefined')` or `if (import.meta.client)`",
        },
    },
    defaultOptions: [],
    create(context) {
